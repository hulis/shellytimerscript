...
let RUUVI_MAC = "xx:xx:xx:xx:xx:xx"; // <-- RuuviTag Bluetooth MAC address

// Heating Thresholds (°C)
let HEATING_ON_THRESHOLD = 21.0;  // Turn heating ON if temperature is below this
let HEATING_TARGET       = 25.0;  // Turn heating OFF once this temperature is reached

// Fallback Settings (If RuuviTag connection is lost)
// Uses a time-based cycle to prevent freezing or overheating
let FALLBACK_ON_TIME = 5 * 60 * 1000;  // 5 min ON
let FALLBACK_OFF_TIME = 15 * 60 * 1000; // 15 min OFF

// BLE Timeout
let BLE_TIMEOUT = 1 * 60 * 1000; // 1 min timeout for sensor data
...
let lastLogTime = 0; // for logger throttling

/***********************
 * SWITCH HELPERS      *
 ***********************/
function heatOn()  { Shelly.call("Switch.Set", { id: 0, on: true }); }
function heatOff() { Shelly.call("Switch.Set", { id: 0, on: false }); }
...
function evaluateSensor() {
  ...
  // Log sensor data max every 30 seconds
  if(Date.now()-lastLogTime>30000){
    console.log("Heating Logic - Temp: " + temperature.toFixed(1) + "°C");
  ...
  if (temperature < HEATING_ON_THRESHOLD) {
    heatOn();
  } else if (temperature >= HEATING_TARGET) {
    heatOff();
  }
}
...
function initBLE(){
  ...
  if(!BLEConfig.enable){ console.log("Error: Bluetooth not enabled in settings"); return; }
...
Timer.set(30000,true,function(){
  if(bleAvailable && (Date.now()-lastSeen>BLE_TIMEOUT)){
    bleAvailable=false; temperature=null;
    evaluateSensor();
  }
});
